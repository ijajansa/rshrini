@extends('layouts.app')
@section('title', 'Update Medium Details')
@section('content')
	...
	<div class="row">
		<div class="col-xl-12">
			<div class="card">
				...
				<div class="card-body">
					<form method="POST" action="{{url('mediums/edit')}}/{{$data->id}}"  enctype="multipart/form-data">
						@csrf
						<div class="row">
								...
								<div class="mb-3">
									...
									<div class="dropdown bootstrap-select default-select form-control wide">
										<select id="inputState" name="standard_id" class="default-select form-control form-control-sm wide @error('standard_id') is-invalid @enderror" tabindex="null">
											...
											<option value="{{$standard->id}}" @if(old('standard_id',$data->standard_id)==$standard->id) selected @endif>{{$standard->name}}</option>
											@endforeach
										</select>
									</div>
									@error('standard_id')
							...
							<div class="col-xl-6 col-sm-6">
								<div class="mb-3">
									...
									<input type="text" class="form-control form-control-sm @error('name') is-invalid @enderror" id="exampleFormControlInput1" value="{{old('name',$data->name)}}" placeholder="e.g. English" name="name">
									@error('name')
									<span class="invalid-feedback" role="alert">
										<strong>{{ $message }}</strong>
									</span>
