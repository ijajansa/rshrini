			...
			<div class="card" id="accordion-four">
				<div class="card-header flex-wrap d-flex justify-content-between px-3">
					<div class="col-lg-6" style="width: 50%">
						<h4 class="card-title">@if(request()->get('format')=='video')Video @elseif(request()->get('format')=='audio')Audio @else PDF @endif List</h4>
					</div>
					<div class="col-lg-6" style="width: 50%;">
...
<script type="text/javascript">
	...
	function datatable()
	{
		...
		var table = $('#myTable').DataTable({
			...
			"ajax":"{{url('chapters/format/all')}}?chapter="+search+"&format="+format,
			"columns":[
			{
				"mData": "id",
			...
			
			{
				...
				"ilter": false,
				"mRender": function (data, type, row) {
					return '<div class="d-flex"><a href="{{url("chapters/format/edit")}}/'+row.id+'" class="btn btn-primary shadow btn-xs sharp me-1"><i class="fas fa-pencil-alt"></i></a><a href="#" onclick="deleteRecord('+row.id+')" class="btn btn-danger shadow btn-xs sharp"><i class="fa fa-trash"></i></a></div>';
	...
	function changeStatus(id)
	{
	    ...
	    {
	        window.location.href="{{url('chapters/format/status')}}/"+id;
	    }
	...
	function deleteRecord(id)
	{
	    ...
	    {
	        window.location.href="{{url('chapters/format/delete')}}/"+id;
	    }
