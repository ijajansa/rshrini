	...
	<div class="row">
		<div class="col-xl-12">
			<div class="card">
				...
				<div class="card-body">
					<form method="POST" action="{{url('chapters/add')}}"  enctype="multipart/form-data">
						@csrf
						<div class="row">
								...
								<div class="mb-3">
									...
									<div class="dropdown bootstrap-select default-select form-control wide">
										<select id="inputState" name="subject_id" class="default-select form-control form-control-sm wide @error('subject_id') is-invalid @enderror" data-search="on" tabindex="null">
											@foreach($data as $ans)
											<option value="{{$ans->id}}" @if(old('subject_id')==$ans->id) selected @endif>{{$ans->name." (".$ans->standard_name." - ".$ans->medium_name.")"}}</option>
											@endforeach
										</select>
									</div>
									@error('subject_id')
